...
7. **Bug real e antigo encontrado ao testar o item 6 acima** (pré-existente, nada a ver com as mudanças de hoje): `getQuestoesAtivasPool()` (duplicada em 5 componentes) buscava `questoes` sem paginar — Supabase corta em 1000 linhas por padrão, e com a tabela em ~6000 questões ativas, qualquer matéria fora das primeiras 1000 retornadas (ordem não garantida) sumia do pool inteiro pra todo mundo, plano pago ou grátis. Achado com "injuria" (1288 questões ativas, 0 nas primeiras 1000). Consolidado numa função paginada única (`buscarQuestoesAtivas` em `lib/questoes-cache.ts`), os 5 componentes agora importam em vez de duplicar. Botão "Verificar" do Treinamento Livre também removido (redundante, "Iniciar" já mostra o resultado real).
8. **Opção de aparecer ou não no ranking público** (`profiles.ranking_visivel`, toggle em `/dashboard/ranking`): substitui a lista fixa de e-mails excluídos do ranking (`docs/ranking-por-materia.sql`) pra casos de conta de teste "poluindo" o Top 5 — a pessoa continua vendo a própria posição/estatísticas mesmo com o ranking desativado, só some da lista pública. Ver `docs/ranking-visibilidade.sql`.
