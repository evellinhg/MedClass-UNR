...
interface Props {
  ...
  /** null cria um novo baralho; um FlashcardDeck existente abre em modo de edição */
  deck: FlashcardDeck | null
  /** ordem sugerida para um baralho novo (maior ordem existente + 1) */
  proximaOrdem: number
  onSaved: () => void
}

export function FlashcardDeckEditDialog({ open, onOpenChange, deck, proximaOrdem, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    const isNew = !deck
    let deckId = deck?.id ?? null
    if (deck) {
      const { error } = await supabase.from("materiais_flashcard_decks").update(deckPayload).eq("id", deck.id)
      if (error) {
