...
export function ResumosGrid() {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from("materiais_resumos").select("*").eq("ativo", true).order("ordem"),
      getPlanStatus(),
    ]).then(([{ data }, planStatus]) => {
      setResumos((data as Resumo[]) ?? [])
      setBloqueado(!!planStatus && !planStatus.canAccessMateriais)
      setLoading(false)
    })
  }, [])
  ...
  return (
    <>
        ...
        {resumos.map((resumo) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() =>
                bloqueado ? alert(t.planRestricted.conteudoBloqueadoAlerta) : setSelectedId(resumo.id)
              }
              className="text-left"
            >
              <Card
                className={`group overflow-hidden border p-0 transition-all ${cor.borderSoft} bg-card ${
                  bloqueado ? "opacity-60" : `${cor.hoverBorder} ${cor.hoverGlow}`
                }`}
              >
                <div className="relative flex h-28 items-center justify-center bg-gradient-to-br from-[#c6ff3a]/15 to-[#84cc16]/15">
                ...
                <div className="space-y-2 p-4">
                  ...
                  {bloqueado ? (
                    <p className="text-xs text-muted-foreground">{t.resumosGrid.bloqueadoTooltip}</p>
                  ) : (
      ...
      {!bloqueado && <ResumoDialog resumoId={selectedId} onClose={() => setSelectedId(null)} />}
    </>
  )
}
