...
export async function getPlanStatus(): Promise<PlanStatus | null> {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("plan, full_name, trial_expires_at, trial_simulados_used, trial_questoes_used")
    .eq("id", user.id)
    .maybeSingle()

  const plan = profile?.plan ?? "gratis"
  const isPaid = plan === "mensal" || plan === "trimestral"
  const hasFullAccess = admin || isPaid

  const trialExpiresAt = profile?.trial_expires_at ?? null
  const isTrialExpired = !hasFullAccess && (!trialExpiresAt || new Date(trialExpiresAt) < new Date())
  ...
  return {
    ...
    canStartSimulado: hasFullAccess || (!isTrialExpired && simuladosRemaining > 0),
    canPracticeIndividual: hasFullAccess || (!isTrialExpired && questoesRemaining > 0),
  }
}
