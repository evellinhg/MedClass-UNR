...
// questões de materia "injuria" (tabela questoes).
//
// Fonte:
// - 111 perguntas flagueadas por tamanho em injuria_flagged.json,
//   distratores reescritos em 3 chunks -> injuria_chunk_NN_fixed.json.
...
const flaggedR2 = JSON.parse(readFileSync(`${SCRATCH}/injuria_flagged.json`, "utf-8"))
const fixedById = new Map()
for (let i = 0; i < 4; i++) {
  const chunk = JSON.parse(readFileSync(`${SCRATCH}/injuria_chunk_${String(i).padStart(2, "0")}_fixed.json`, "utf-8"))
...
console.log(`Distratores da 2ª rodada aplicados: ${totalDistratoresAplicados} (esperado 311)`)
...
console.log(`\nConcluído: ${feitos} questões de injuria atualizadas (2ª rodada, cobertura completa).`)
