// Script de uso único: importa examen_cirugia.json (20 questões de Cirurgia).
// O arquivo veio com "parcial": "Segundo Parcial", mas Cirurgia só tem um
// parcial na grade -- força "Primer Parcial" independente do rótulo de origem.
// Alternativas vêm em dict A-D (não na lista {letra,texto,...} que o
// importador padrão espera) e "tags" vem como string separada por vírgula.
//
// Uso: node scripts-tmp-importar-cirurgia.mjs
...
const PARCIAL_KEY = "parcial1" // Cirurgia tem só um parcial -- ignora o "Segundo Parcial" do arquivo.
...
function converterItem(item) {
  const dificuldadeKey = DIFICULDADE_LABEL_TO_KEY[item.dificuldade ?? item.dificultad]
  if (!dificuldadeKey) throw new Error(`Dificuldade não reconhecida: "${item.dificuldade}"`)
  ...
  const alternativas = Object.entries(item.alternativas ?? {})
    .map(([letra, texto]) => {
      ...
      const entrada = Object.entries(item.feedback ?? {}).find(([k]) => k.toLowerCase() === letra.toLowerCase())
...
const raw = JSON.parse(readFileSync(`${process.env.HOME}/Downloads/examen_cirugia.json`, "utf-8"))
