// Importa o lote de Oftalmologia (4º ano) espalhado em varios arquivos na
// pasta Downloads. Pula duplicatas exatas (mesmo md5) e o arquivo
// perguntas_oftalmologia.json, que veio com conteudo contaminado por
// portugues (traducao malfeita misturada com o espanhol original).
...
const ARQUIVOS = [
  "banco_preguntas_oftalmologia_90.json",
  "examen_oftalmologia_100_preguntas_detalladas.json",
  "examen_oftalmologia_20_preguntas_detalladas.json",
  "examen_oftalmologia_90_preguntas.json",
  "examen_oftalmologia_90_preguntas-v3.json",
  "perguntas_oftalmologia_v2.json",
  "preguntas_examen_oftalmologia.json",
  "preguntas_examen_oftalmologia_septiembre_2025.json",
  "preguntas_exclusivas_oftalmologia.json",
]
...
const PARCIAL_LABEL_TO_KEY = {
  "Primeira Parcial": "parcial1", "Primer Parcial": "parcial1", "Primeiro Parcial": "parcial1", "Primero Parcial": "parcial1",
...
function mapQuestao(q, origem) {
  ...
  // Alguns arquivos vem com tags como string "a, b, c" em vez de array --
  // sem normalizar, o Postgres rejeita o insert com "malformed array literal".
...
for (const nomeArquivo of ARQUIVOS) {
  const raw = JSON.parse(readFileSync(`${HOME}/Downloads/${nomeArquivo}`, "utf-8"))
  const lista = extrairLista(raw)
  for (const q of lista) {
    try {
      mapeadas.push(mapQuestao(q, nomeArquivo))
    } catch (e) {
