...
// das alternativas (viés de letra, forte em direção a "A": 29,9%) nas 415
// questões de materia "nutricao" (tabela questoes).
//
// Fonte:
// - 71 perguntas flagueadas por tamanho em nutricao_flagged.json,
//   distratores reescritos em 2 chunks -> nutricao_chunk_NN_fixed.json.
// - Reembaralhamento por round-robin de letra alvo, agrupado por nº de
//   opções (2 a 7), pra balancear a letra correta dentro de cada grupo.
//
// Uso: node scripts-tmp-atualizar-nutricao.mjs --dry-run
//      node scripts-tmp-atualizar-nutricao.mjs
...
// -- carrega e mescla os 2 chunks corrigidos ---------------------------------
const flagged = JSON.parse(readFileSync(`${SCRATCH}/nutricao_flagged.json`, "utf-8"))
const fixedById = new Map()
for (let i = 0; i < 2; i++) {
  const chunk = JSON.parse(readFileSync(`${SCRATCH}/nutricao_chunk_${String(i).padStart(2, "0")}_fixed.json`, "utf-8"))
...
const { data: rows, error } = await supabase.from("questoes").select("*").eq("materia", "nutricao")
...
console.log(`Distratores aplicados: ${totalDistratoresAplicados} (esperado 122)`)
...
console.log(`\nConcluído: ${feitos} questões de nutricao atualizadas.`)
