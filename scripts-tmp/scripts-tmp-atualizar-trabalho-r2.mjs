// Segunda passada de correção pra "trabalho_tempo_livre" (tabela questoes).
// A primeira passada (scripts-tmp-atualizar-trabalho.mjs) usou select("*")
// sem paginação e só processou 1000 das 1286 questões da matéria (limite
// padrão de linhas do Supabase) -- as 286 restantes nunca foram analisadas,
// flagueadas nem reembaralhadas.
//
// Esta segunda passada:
// 1. Aplica os 115 distratores recém-expandidos (55 perguntas que tinham
//    ficado de fora) de trabalho_r2_chunk_NN_fixed.json.
// 2. Busca TODAS as 1286 linhas com paginação (PAGE=1000) e refaz o
//    reembaralhamento completo (round-robin por grupo de nº de opções),
//    cobrindo agora o conjunto inteiro -- isso reembaralha de novo as 1000
//    já tratadas na 1ª passada também, o que não tem problema (idempotente
//    em termos de correção; só muda a ordem de novo) e garante que a
//    distribuição de letra final seja balanceada sobre as 1286, não só 1000.
//
// Uso: node scripts-tmp-atualizar-trabalho-r2.mjs --dry-run
//      node scripts-tmp-atualizar-trabalho-r2.mjs
...
const MATERIA = "trabalho_tempo_livre"
...
// -- carrega e mescla os 2 chunks da 2ª rodada -------------------------------
const flaggedR2 = JSON.parse(readFileSync(`${SCRATCH}/trabalho_flagged_round2.json`, "utf-8"))
const fixedById = new Map()
for (let i = 0; i < 2; i++) {
  const chunk = JSON.parse(readFileSync(`${SCRATCH}/trabalho_r2_chunk_${String(i).padStart(2, "0")}_fixed.json`, "utf-8"))
...
console.log(`Distratores da 2ª rodada aplicados: ${totalDistratoresAplicados} (esperado 115)`)
...
console.log(`\nConcluído: ${feitos} questões de trabalho_tempo_livre atualizadas (2ª rodada, cobertura completa).`)
