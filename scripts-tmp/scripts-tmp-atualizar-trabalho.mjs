...
// das alternativas (viés de letra, forte em direção a "B": 39,3%) nas 1000
// questões de materia "trabalho_tempo_livre" (tabela questoes).
//
// Fonte:
// - 161 perguntas flagueadas por tamanho em trabalho_flagged.json,
//   distratores reescritos em 4 chunks -> trabalho_chunk_NN_fixed.json.
// - Reembaralhamento por round-robin de letra alvo, agrupado por nº de
//   opções (2, 3, 4 ou 5), pra balancear a letra correta dentro de cada grupo.
//
// Uso: node scripts-tmp-atualizar-trabalho.mjs --dry-run
//      node scripts-tmp-atualizar-trabalho.mjs
...
const flagged = JSON.parse(readFileSync(`${SCRATCH}/trabalho_flagged.json`, "utf-8"))
const fixedById = new Map()
for (let i = 0; i < 4; i++) {
  const chunk = JSON.parse(readFileSync(`${SCRATCH}/trabalho_chunk_${String(i).padStart(2, "0")}_fixed.json`, "utf-8"))
...
const { data: rows, error } = await supabase.from("questoes").select("*").eq("materia", "trabalho_tempo_livre")
...
console.log(`Distratores aplicados: ${totalDistratoresAplicados} (esperado 339)`)
...
console.log(`\nConcluído: ${feitos} questões de trabalho_tempo_livre atualizadas.`)
