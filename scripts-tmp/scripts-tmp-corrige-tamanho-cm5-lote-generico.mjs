...
const loteNum = process.argv[2]
if (!loteNum) { console.error("uso: node scripts-tmp-corrige-tamanho-cm5-lote-generico.mjs <numero-do-lote-2-digitos>"); process.exit(1) }
const lote = JSON.parse(readFileSync(`${SCRATCH}/cm5_lote_${loteNum}.json`, "utf-8"))
...
console.log(`Lote ${loteNum} — Reforço: questões editadas =`, updates1.length)
...
console.log(`Lote ${loteNum} — Poda: questões editadas =`, updates2.length)
...
console.log(`\nLote ${loteNum} FINAL: mais_longa=${aindaLonga} (${(100*aindaLonga/pos2.length).toFixed(1)}%), mais_curta=${aindaCurta} (${(100*aindaCurta/pos2.length).toFixed(1)}%), equilibrada=${ok2}`)
console.log(`Razão média: ${(ratios.reduce((a,b)=>a+b,0)/ratios.length).toFixed(2)}`)

let integroOk = true
for (const q of pos2) {
  const original = byId[q.id]
  if (q.opcoes[q.indice_correta] !== original.opcoes[original.indice_correta]) { console.error("ALTERAÇÃO INDEVIDA:", q.id); integroOk = false }
}
